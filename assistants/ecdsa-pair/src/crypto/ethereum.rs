...
pub const CRYPTO_ID: CryptoTypeId = CryptoTypeId(*b"ethm");
...
mod test {
    ...
    fn ss58check_format_check_works() {
        ...
        assert_eq!(
            Public::from_ss58check_with_version(&s),
            Err(PublicError::FormatNotAllowed)
        );
    }
    ...
    fn sign_prehashed_works() {
        ...
        let sig2: Signature = {
            ...
            SECP256K1
                .sign_ecdsa_recoverable(&message, &pair.secret)
                .into()
        };
        assert_eq!(sig1, sig2);
