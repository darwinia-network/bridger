...
async fn run_update_fee(config_task: TaskConfig) -> color_eyre::Result<()> {
    let bridge_config: DarwiniaCrabConfig = Config::restore(Names::BridgeDarwiniaCrab)?;
    let subscan_config_darwinia: Option<SubscanConfig> = bridge_config.darwinia_subscan;
    let subscan_config_crab: Option<SubscanConfig> = bridge_config.crab_subscan;
