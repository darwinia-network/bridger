...
pub async fn exec(network: String,  txblock: u64, mmrblock: u64, signblock: u64) -> Result<()> {
    ...
    let proof = darwinia2ethereum.get_headermmr_genproof(txblock, mmrblock, header.hash()).await?;
    let event_proof = darwinia.get_event_proof(hex::decode("f8860dda3d08046cf2706b92bf7202eaae7a79191c90e76297e0895605b8b457").unwrap(), header.hash()).await?;
    ...
    // confirmed block
    let events = darwinia.get_events_from_block_number(signblock as u32).await?;
    for event in events {
