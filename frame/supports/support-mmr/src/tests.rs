...
mod tests {
    use super::MergeHash;
    use crate::mmr::{bag_rhs_peaks, gen_proof, gen_proof_positions, get_peaks, leaf_index_to_mmr_size};
    ...
    fn test_mmr_proof() {
        ...
        while verified_number < leaf_index {
            ...
            // 1. gen positions
            let (merkle_proof_pos, peak_positions, peak_pos_of_leaf_index) = gen_proof_positions(verified_position, mmrsize);
            ...
            assert_eq!(true, mmr_proof_expected.verify(root, vec![(verified_position, hash(&verified_number.to_le_bytes()))]).unwrap());
            let mmr_proof_instance = cmmr::MerkleProof::<[u8;32], MergeHash>::new(mmrsize, mmr_proof);
            assert_eq!(true, mmr_proof_instance.verify(root, vec![(verified_position, hash(&verified_number.to_le_bytes()))]).unwrap());
            verified_number += 1;
        }
    }
}
