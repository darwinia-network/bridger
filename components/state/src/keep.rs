...
pub fn put_task_password(task: impl AsRef<str>, password: impl AsRef<str>) -> anyhow::Result<()> {
...
pub fn remove_task_password(task: impl AsRef<str>) -> anyhow::Result<()> {
    let mut map = TASK_PASSWORD_MAP
        .lock()
        .map_err(|_e| StandardError::Api("Failed to get task password instance".to_string()))?;
...
pub fn get_task_password(task: impl AsRef<str>) -> anyhow::Result<Option<String>> {
