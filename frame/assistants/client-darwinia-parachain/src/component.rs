...
/// Subxt component
pub struct DarwiniaParachainClientComponent;

impl DarwiniaParachainClientComponent {
    /// Get subxt client instance
    pub async fn component(config: ClientConfig) -> ClientResult<DarwiniaParachainClient> {
        let mut attempts = 1;
        let mut wait_secs = 1;
        ...
        loop {
            ...
            return match ClientBuilder::new().set_url(&endpoint).build().await {
                Ok(client) => Ok(DarwiniaParachainClient::new(client, account.clone())),
                Err(err) => {
