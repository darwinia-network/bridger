...
use substrate_subxt::{sp_core::H256, sp_runtime::traits::{BlakeTwo256, Hash}};
use std::str::FromStr;
use hex::ToHex;
...
impl Into<Option<HeaderMMR>> for HeaderMMRRpc {
    fn into(self) -> Option<HeaderMMR> {
        ...
        let trim: &[_] = &['[', ']'];
        let proof: Vec<String> = self.proof.trim_matches(trim).split(',').map(|s|String::from(s.trim())).collect();
        Some(HeaderMMR {
            block: 0,
...
fn convert(mmr_size: u64, block: u64, block_hash: H256, mmr_proof: &Vec<String>) -> (Vec<String>, Vec<String>) {
    ...
    let (peak_pos, last, res_siblings) = if res_peaks.len() + 1 == peaksize {
        (peaks[res_peaks.len()], None, mmr_proof[res_peaks.len()..].to_vec())
    } else {
        (peaks[res_peaks.len()], Some(&mmr_proof[mmr_proof.len()-1]), mmr_proof[res_peaks.len()..mmr_proof.len()-1].to_vec())
    };
    let proof: Vec<H256> = mmr_proof_iter.map(|hash| H256::from_str(&hash[2..]).unwrap()).collect();
...
mod mmr_convert_test {
	...
	fn test_convert() {
        ...
        header.hash = H256::from_str("5431b121fb0549830b7f33865d290eea566cc1ae83b260c2edc8b44cfc9115d3").unwrap();
        println!("{:?}", header);

        let formated_mmr: Option<FormatedMMR> = header.into();
        if let Some(mmr_proof) = formated_mmr {
            assert_eq!("0x1c7d559a2231da8354399af954f8faa205f410995deba7fd25086dcb663a77ec", mmr_proof.peaks[&mmr_proof.peaks.len()-2]);
        }
	}
}
