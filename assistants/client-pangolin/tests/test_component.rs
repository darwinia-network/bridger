...
async fn test_system_number() {
    let client = common::client().await.unwrap();
    let address = client_pangolin::runtime_api::storage().system().number();
    let number = client
        .subxt()
        .storage()
...
async fn test_technical_committee_members() {
    let client = common::client().await.unwrap();
    let address = client_pangolin::runtime_api::storage()
        .technical_committee()
        .members();
    let members = client
        .subxt()
...
async fn test_transfer_call() {
    ...
    let call = client_pangolin::runtime_api::tx().balances().transfer(
...
async fn test_subscribe() {
    ...
    use client_pangolin::types::runtime_types::{
