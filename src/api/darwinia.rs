//! Darwinia API
use crate::{
	memcache::EthereumTransaction,
	result::{Error::Bridger, Result},
	Config,
};
...
impl Darwinia {
	...
	pub async fn affirm(&self, parcel: EthereumRelayHeaderParcel) -> Result<H256> {
		match &self.account.real {
			...
			None => Ok(self
				.client
				.affirm(&self.account.signer, parcel, None)
				.await?),
		}
	}
	...
	pub async fn redeem(
		...
		match &self.account.real {
			...
			None => Ok(self
				.client
				.redeem(&self.account.signer, redeem_for, proof)
				.await?),
		}
	}
