    ...
    match microkv::MicroKV::open_with_base_path(dbname.clone(), config.base_path.clone()) {
        ...
        Err(e) => match e.error {
            microkv::errors::ErrorType::MigrateError(from, to) => {
                ...
                Err(
                    StateComponentError::Microkv(format!("Failed migrate microkv, {:?}", e.msg))
                        .into(),
                )
            }
            _ => Err(StateComponentError::Microkv(format!(
...
// todo: maybe in version 0.5.x don't need to do this migration
fn try_microkv_migrate_less030_to_03x(
    dbname: String,
    config: &MicrokvConfig,
) -> color_eyre::Result<microkv::history::MicroKV030> {
