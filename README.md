...
`bridger run 2>&1 | logtrim` will run the bridger fully. You can also using ```bridger run -v``` to enter verbose model which can help print more trace logs.

`bridger guard 2>&1 | logtrim` will run the guard service standalone.
