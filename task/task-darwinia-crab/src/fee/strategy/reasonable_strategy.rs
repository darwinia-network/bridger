...
impl ReasonableStrategy {
    pub async fn new() -> anyhow::Result<Self> {
        ...
        let subscan_pangolin = component_subcan_darwinia.component().await?;
        let subscan_pangoro = component_subscan_crab.component().await?;
        Ok(Self {
            helper,
            subscan_pangolin,
            subscan_pangoro,
...
impl UpdateFeeStrategy for ReasonableStrategy {
    async fn handle(&self) -> anyhow::Result<()> {
        let top100_pangolin = self.subscan_darwinia.extrinsics(1, 100).await?;
        let top100_pangoro = self.subscan_crab.extrinsics(1, 100).await?;
        let top100_pangolin = top100_darwinia.data()?.ok_or_else(|| {
            StandardError::Api("Can not query pangolin extrinsics data".to_string())
        })?;
        let top100_pangoro = top100_crab.data()?.ok_or_else(|| {
