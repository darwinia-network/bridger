...
async fn start() -> color_eyre::Result<()> {
    ...
    loop {
        if let Err(error) = update_manager.sync_committee_update().await {
            tracing::error!(
                target: "darwinia-eth",
                "[SyncCommittee][Eth=>Darwinia] Failed relay sync committee update : {:?}",
                error
            );
            return Err(error);
        }
...
impl SyncCommitteeUpdate {
    pub async fn sync_committee_update(&self) -> color_eyre::Result<()> {
        ...
        if next_sync_committee.is_zero() {
            tracing::info!(
                target: "darwinia-eth",
                "[SyncCommittee][Eth=>Darwinia] Try to relay SyncCommittee at period {:?}",
                period + 1,
            );
            ...
            tracing::info!(
                target: "darwinia-eth",
                "[SyncCommittee][Eth=>Darwinia] Sending tx: {:?}",
                &tx
            );
            ...
            tracing::info!(
                target: "darwinia-eth",
                "[SyncCommittee][Eth=>Darwinia] Next sync committee is {:?}",
                next_sync_committee
            );
        }
        Ok(())
