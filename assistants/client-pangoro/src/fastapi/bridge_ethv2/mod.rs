...
impl EcdsaClient for PangoroClient {
    ...
    async fn is_ecdsa_authority(
        ...
        let hash = self
            ...
            .await?;
        tracing::trace!(target: "client-pangoro", "[is_ecdsa_authority] Block hash: {:?}", hash);
        let authorities = crate::subxt_runtime::api::storage()
            .ecdsa_authority()
            .authorities();
        let authorities = self.subxt().storage().fetch(&authorities, hash).await?;
        tracing::trace!(target: "client-pangoro", "[is_ecdsa_authority] Authorities fetched");
        match authorities {
            None => Ok(false),
            Some(authorities) => {
    ...
    async fn submit_authorities_change_signature(
        ...
        let events = track.wait_for_finalized_success().await.map_err(|e| {
            E2EClientError::Custom(format!("send transaction failed pangoro: {:?}", e))
        })?;
    ...
    async fn submit_new_message_root_signature(
        ...
        tracing::trace!(target: "client-pangoro", "[submit_new_message_root_signature] tx hash: {:?}", hash);
        let events = track.wait_for_finalized_success().await.map_err(|e| {
            E2EClientError::Custom(format!("send transaction failed pangoro: {:?}", e))
        })?;
        Ok(events.extrinsic_hash())
